...
describe('plugin', function () {
    it('should work as a remarkable plugin', function () {
        ...
        var actual = render(read('./fixtures/strip-words.md'), {
            slugify: false,
            strip: function (str) {
                return '~' + str.slice(4) + '~';
            },
...
describe('options: custom functions:', function () {
    it('should allow a custom `strip` function to strip words from heading text:', function () {
        var actual = toc(read('./fixtures/strip-words.md'), {
            slugify: false,
            strip: function (str) {
                return '~' + str.slice(4) + '~';
            },
    ...
    it('should allow a `filter` function to filter out unwanted bullets:', function () {
        var actual = toc(read('./fixtures/filter.md'), {
...
describe('toc', function () {
    it('should generate a TOC from markdown headings:', function () {
        assert.equal(toc('# AAA\n# BBB\n# CCC\nfoo\nbar\nbaz').content, ['- [AAA](#aaa)', '- [BBB](#bbb)', '- [CCC](#ccc)'].join('\n'));
    });
    ...
    it('should use a different bullet for each level', function () {
        assert.equal(toc(read('./fixtures/levels.md')).content, [
            '- [AAA](#aaa)',
            '  * [a.1](#a1)',
            '    + [a.2](#a2)',
            '      - [a.3](#a3)',
    ...
    it('should use a different bullet for each level', function () {
        assert.equal(toc(read('./fixtures/repeat-bullets.md')).content, [
            '- [AAA](#aaa)',
            '  * [a.1](#a1)',
            '    + [a.2](#a2)',
            '      - [a.3](#a3)',
    ...
    it('should handle mixed heading levels:', function () {
        assert.equal(toc(read('./fixtures/mixed.md')).content, [
            '- [AAA](#aaa)',
            '  * [a.1](#a1)',
            '    + [a.2](#a2)',
            '      - [a.3](#a3)',
    ...
    it('should ignore headings in fenced code blocks.', function () {
        assert.equal(toc(read('./fixtures/fenced-code-blocks.md')).content, [
            '- [AAA](#aaa)',
            '  * [a.1](#a1)',
            '    + [a.2](#a2)',
            '      - [a.3](#a3)',
    ...
    it('should rotate bullets when there are more levels than bullets defined:', function () {
        var actual = toc(read('./fixtures/repeated-headings.md')).content;
        assert.equal(actual, read('./expected/repeated-headings.md'));
    });
    it('should wrap around the bullet point array', function () {
        var actual = toc(read('./fixtures/heading-levels.md'), {
            bullets: ['*', '-'],
        });
        assert.equal(actual.content, [
            '* [AAA](#aaa)',
    ...
    it('should remove diacritics from the links', function () {
        var actual = toc(read('./fixtures/diacritics.md'));
        assert.equal(actual.content, [
...
describe('toc.insert', function () {
    it('should retain trailing newlines in the given string', function () {
        var str = fs.readFileSync('./fixtures/newline.md', 'utf8');
        assert.equal(toc.insert(str), fs.readFileSync('./expected/newline.md', 'utf8'));
    });
    it('should insert a markdown TOC beneath a `<!-- toc -->` comment.', function () {
        var str = read('./fixtures/insert.md');
        assert.equal(strip(toc.insert(str)), read('./expected/insert.md'));
    });
    it('should replace an old TOC between `<!-- toc -->...<!-- tocstop -->` comments.', function () {
        var str = read('./fixtures/replace-existing.md');
        assert.equal(strip(toc.insert(str)), read('./expected/replace-existing.md'));
    });
    it('should insert the toc passed on the options.', function () {
        var str = read('./fixtures/replace-existing.md');
        assert.equal(strip(toc.insert(str, { toc: toc(str).content })), read('./expected/replace-existing.md'));
        assert.equal(strip(toc.insert(str, { toc: '- Foo' })), read('./expected/foo.md'));
    });
    it('should accept options', function () {
        var str = read('./fixtures/insert.md');
        var actual = toc.insert(str, {
            slugify: function (text) {
                return text.toLowerCase();
            },
        });
        assert.equal(strip(actual), read('./expected/insert-options.md'));
    });
    it('should accept no links option', function () {
        var str = read('./fixtures/insert.md');
        assert.equal(strip(toc.insert(str, {})), read('./expected/insert.md'));
        assert.equal(strip(toc.insert(str, { linkify: true })), read('./expected/insert.md'));
        assert.equal(strip(toc.insert(str, { linkify: false })), read('./expected/insert-no-links.md'));
    });
});
