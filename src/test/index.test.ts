...
import {insepct} from 'util'
import {getTitle, slugify, stripColor} from '../utils'
...
describe('options: custom functions:', function () {
  ...
  it('should allow a `filter` function to filter out unwanted bullets:', function () {
    var actual = toc(read('./fixtures/filter.md'), {
      filter: function (str: string, ele: any, arr: any) {
...
describe('toc', function () {
  ...
  it('should use a different bullet for each level', function () {
    assert.equal(
      toc(read('./fixtures/levels.md')).content,
      [
        '- [AAA](#aaa)',
        '  * [a.1](#a1)',
        '    + [a.2](#a2)',
  ...
  it('should use a different bullet for each level', function () {
    assert.equal(
      toc(read('./fixtures/repeat-bullets.md')).content,
      [
        '- [AAA](#aaa)',
        '  * [a.1](#a1)',
        '    + [a.2](#a2)',
  ...
  it('should handle mixed heading levels:', function () {
    assert.equal(
      toc(read('./fixtures/mixed.md')).content,
      [
        '- [AAA](#aaa)',
        '  * [a.1](#a1)',
        '    + [a.2](#a2)',
  ...
  it('should ignore headings in fenced code blocks.', function () {
    assert.equal(
      toc(read('./fixtures/fenced-code-blocks.md')).content,
      [
        '- [AAA](#aaa)',
        '  * [a.1](#a1)',
        '    + [a.2](#a2)',
...
describe('toc.insert', function () {
  it('should retain trailing newlines in the given string', function () {
    ...
    assert.equal(
      toc.insert(str),
      fs.readFileSync('./expected/newline.md', 'utf8'),
    )
  })
  ...
  it('should insert the toc passed on the options.', function () {
    var str = read('./fixtures/replace-existing.md')
    assert.equal(
      strip(toc.insert(str, {toc: toc(str).content})),
      read('./expected/replace-existing.md'),
    )
    assert.equal(
      strip(toc.insert(str, {toc: '- Foo'})),
      read('./expected/foo.md'),
    )
  })
  ...
  it('should accept no links option', function () {
    ...
    assert.equal(strip(toc.insert(str, {})), read('./expected/insert.md'))
    assert.equal(
      strip(toc.insert(str, {linkify: true})),
      read('./expected/insert.md'),
    )
    assert.equal(
      strip(toc.insert(str, {linkify: false})),
      read('./expected/insert-no-links.md'),
    )
  })
})
